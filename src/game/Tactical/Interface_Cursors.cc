...
const UICursor gUICursors[NUM_UI_CURSORS] =
{
	...
	{MOVE_REALTIME_UICURSOR,             UICURSOR_FREEFLOWING | UICURSOR_SHOWTILEAPDEPENDENT | UICURSOR_DONTSHOW2NDLEVEL,                      VIDEO_DEFAULT_TO_NO_CURSOR,           FIRSTPOINTERS2  }, //Fluffy (ForeverMouseCursor): Changed VIDEO_NO_CURSOR to VIDEO_DEFAULT_TO_NO_CURSOR
...
void DrawUICursor()
{
	...
	if (usMapPos != NOWHERE)
	{
		...
		{
			gViewportRegion.ChangeCursor(VIDEO_DEFAULT_TO_NO_CURSOR); //Fluffy (ForeverMouseCursor)
			return;
		}
